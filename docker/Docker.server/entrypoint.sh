...
function mockdata {
    if ! ./manage.py dumpdata auth | grep -q interview; then
      ...
      python manage.py loaddata cardgame/fixtures/users.json
    else
...
function test {
    ...
    cd /code/server/
    if [ $# -eq 0 ]; then
        DJANGO_SETTINGS_MODULE=cardgame.settings pytest tests/
    else
        DJANGO_SETTINGS_MODULE=cardgame.settings pytest "$@"
    fi
}
